# Copy of the vscode home manager module that works for cursor instead
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    home.file = mkMerge [
      ...
      (mkIf (cfg.userTasks != { }) {
        "${tasksFilePath}".source = jsonFormat.generate "vscode-user-tasks" cfg.userTasks;
      })
      (mkIf (cfg.keybindings != [ ]) (
        ...
        in
        {
          "${keybindingsFilePath}".source = jsonFormat.generate "vscode-keybindings" (
      ...
      (mkIf (cfg.extensions != [ ]) (
        ...
        in
        if cfg.mutableExtensionsDir then
          mkMerge (
            concatMap toPaths cfg.extensions
            ++ lib.optional (true) {
            ...
            "${extensionPath}".source =
              let
                combinedExtensionsDrv = pkgs.buildEnv {
                  ...
                  paths = cfg.extensions ++ lib.optional (true) extensionJsonFile;
                };
              in
      ...
      (mkIf (cfg.globalSnippets != { }) (
        ...
        in
        {
          "${globalSnippets}".source =
            jsonFormat.generate "user-snippet-global.code-snippets" cfg.globalSnippets;
        }
      ))
